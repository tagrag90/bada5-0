...
const streamServerClient = {
  ...
  upsertUser: async ({ id, username, name }: { id: string; username: string; name: string }) => {
    logger.info(`[DUMMY] Stream upsertUser called for ${username}`);
    return { id, name, username };
  },
  
  // 사용자 부분 업데이트
  partialUpdateUser: async ({ id, set }: { id: string; set: any }) => {
    logger.info(`[DUMMY] Stream partialUpdateUser called for ${id}`);
    return { id, ...set };
  },
  
  // 토큰 생성
  createToken: (userId: string, expiresAt: number, issuedAt: number) => {
    logger.info(`[DUMMY] Stream createToken called for ${userId}`);
    return "dummy_stream_token";
  },
  
  // 읽지 않은 메시지 수 가져오기
  getUnreadCount: async (userId: string) => {
    logger.info(`[DUMMY] Stream getUnreadCount called for ${userId}`);
